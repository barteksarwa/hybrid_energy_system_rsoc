...
(li_ion_capacity, power, sofc_power, soec_power, battery_power, load, SoCH2,
 EMS_State, net_power, deficit_energy, loss_energy) = np.loadtxt("result_article_before\\before.csv", unpack=True)
t = np.linspace(0,len(time),len(time))
net = power + sofc_power - soec_power + battery_power - load
def adjust_time_ranges(time_ranges, n):
    I = 24 * n
    return [(start + I, end + I) for start, end in time_ranges]
n = 11

time_ranges = [
    (1656,1680),       # March 10
    (3864,3888),            # June 10
    (6072,6096),            # September 10
    (8256,8280)             # December 10
]
#
# time_ranges = [
#     (1656,1680),
...
adjusted_time_ranges = adjust_time_ranges(time_ranges, n)

# 2x2 plots
fig = plt.figure(figsize=(InitParams.fig_width, InitParams.fig_height))
gs = fig.add_gridspec(4, 2, height_ratios=[1.1, 3, 1.1, 3])

# Iterate through each time range and create the subplots
for idx, (start, stop) in enumerate(adjusted_time_ranges):
    row = idx // 2
    col = idx % 2
    ...
    ax1.plot(time[start:stop], power[start:stop], color='gold', label='Generated PV energy [W]')
    ax1.plot(time[start:stop], load[start:stop], color='darkorchid', linewidth=1.2, label='Load demand [W]')
    ax1.plot(time[start:stop], sofc_power[start:stop], color='red', label='SOFC energy [W]')
    ax1.plot(time[start:stop], soec_power[start:stop], color='lime', label='SOEC energy [W]')
    ax1.plot(time[start:stop], battery_power[start:stop], color='#000080', label='Battery energy [W]')
    ax1.plot(time[start:stop], net[start:stop], color='pink', linewidth=2, label='Battery energy [W]')
    ax1.set_xlabel(date_label)
    ax1.set_ylabel('Energy [W]', color='black')
    ...
    ax2.set_ylabel('$SoC$ [-]', color='black', labelpad=18, bbox=dict(facecolor='none', edgecolor='none', pad=50))
    ax2.set_ylim([0, 1.05])
    ...
    ax1.xaxis.set_major_locator(mdates.HourLocator(range(0, 24), 8))
...
plt.legend(
    combined_handles, combined_labels, loc=9, bbox_to_anchor=(-0.2, -0.8),
    fancybox=False, shadow=False, ncol=4
)

# Set overall title
# plt.suptitle('Simulation of the microgrid --- optimization by the F4 function', y=0.98)
...
# Show the plot
# plt.show()
fig.savefig("combinedfourdays_before.pdf", format='pdf', bbox_inches='tight')
